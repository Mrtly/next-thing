...
export default function D3Map() {
  ...
  useEffect(() => {
    d3.json(countyDataUrl).then((data, error) => {
      if (error) {
        console.log(error);
      } else {
    ...
    d3.json(eduDataUrl).then((data, error) => {
      if (error) {
        console.log(error);
      } else {
        let eData = data;
  ...
  return (
    <Card>
      ...
      <div className="text-sm font-thin">
        shows % of people with a Bachelors or higher education per county
      </div>
      <svg ref={svgRef} />
      <div
        ref={tooltipRef}
        className="absolute bg-white px-6 py-3 font-medium tracking-wider"
      />
      <div className="mt-2 text-sm font-thin">
        project idea & sample data (
        <a
          target="_blank"
          href={countyDataUrl}
        ...
        ,{" "}
        <a target="_blank" href={eduDataUrl} className="text-coral underline">
          education
        </a>
        ) from freeCodeCamp
      </div>
