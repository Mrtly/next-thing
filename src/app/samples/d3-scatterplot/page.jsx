...
export default function D3Scatter() {
  ...
  const drawViz = () => {
    ...
    svg.selectAll('circle')
      ...
      .attr('class', (item) => { return item['Doping'] !== '' ? 'fill-yin hover:fill-violet' : 'fill-gold hover:fill-violet' })
      .on('mouseover', (x, item) => { 
    ...
    <Card>
      <div className="px-8 py-4 flex flex-col">
        ...
        <Card id="legend" className="mt-2 p-2 text-sm font-thin self-end">
          <span className='text-yin'>Purple = Doping Allegation </span> | <span className='text-gold'>Gold = No Doping Allegation</span>
        </Card>
        <svg ref={svgRef}/>
        <div ref={tooltipRef} className='h-14 py-2 px-4 bg-ground bg-opacity-25 text-sm'/>
      </div>
        <div className="text-sm font-thin">project idea & <a target='_blank' href={source} className='underline text-coral'>sample data</a> from freeCodeCamp</div>
    </Card>
  );
}
